"""
Batch scrape all South American countries sequentially.
Skips countries that already have a completed output file.
Run: python scrape_south_america.py
"""
import subprocess
import sys
import os
...
# South American countries (ordered by estimated size, smaller first)
SOUTH_AMERICA_COUNTRIES = [
    # Small territories
    "Falkland Islands", "French Guyana", "French South and Antarctic Terr.",
    "British Antarctic", "Guyana", "Suriname",
    
    # Medium-sized countries
    "Uruguay", "Paraguay", "Bolivia", "Ecuador", "Peru",
    
    # Larger countries
    "Chile", "Colombia", "Venezuela", "Argentina", "Brazil",
]
...
def main():
    total = len(SOUTH_AMERICA_COUNTRIES)
    completed = 0
    failed = 0
    
    for i, country in enumerate(SOUTH_AMERICA_COUNTRIES, 1):
        if is_done(country):
